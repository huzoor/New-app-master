...
import productViewUtils from './productViewUtils';
import {
  getDealerRates,
...
import {
  ...
  getProductRateKey,
  getSelectedProvider,
  getUpdatedSelectedProductInfo,
  getUpdatesProductsList,
...
class Product extends React.Component {
  ...
  componentWillMount() {
    ...
    if (this.state.slectedProductInfo.is_rateable) {
      ...
      console.log('Selected Product Inf ***', this.state)
      if(this.state.isPlan1Selected || this.state.isPlan2Selected || this.state.isPlan3Selected || this.state.isPlan4Selected){
        let currentSelectedProduct = this.state.slectedProductInfo;
        this.props.dispatch(getDealerRatesFromDB(cprovider.providerId, provider.product_code, provider.provider_code, this.state.slectedProductInfo.id, urrentSelectedProduct))
  ...
  getRates(event) {
    ...
    let updatedSelectedProductInfo = getUpdatedSelectedProductInfo(this.state, selectedProvider)
    ...
    let updatesProductsList = getUpdatesProductsList(existingProductsList, selectedProvider);
