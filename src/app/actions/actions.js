...
export function getDealerRatesFromDb(providerId, productCode, providerCode, id, selectedProduct){
  return(dispatch, getState)=>{
    ...
    let ratesData = {
      ...
      "deal_id": selectedProduct.dealer_id,
    ...
    populateDealerData();
    let providerRateUrl = `${config.emenuMobileGatewayAPI}/deal-jackets/${dealerData.dealjacketid}/deals/${selectedProduct.dealer_id}/provider-product-rates/${selectedProduct.provider_code}/${selectedProduct.category_code}/`;
    httpHelper(providerRateUrl, 'get', ratesData).then((data) => {
      dispatch({
        type: GET_RATES,
        payload: {
          data,
